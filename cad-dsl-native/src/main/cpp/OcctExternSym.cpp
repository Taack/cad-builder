...
extern "C" TopoDS_Shape* brep_builderapi_transform_shape(const TopoDS_Shape& shape, const gp_Trsf& gp_trsf, const Standard_Boolean theCopyGeom)
{
    return new TopoDS_Shape(BRepBuilderAPI_Transform(shape, gp_trsf).Shape());
}

extern "C" TopoDS_Shape* brep_algoapi_cut(TopoDS_Shape& result, const TopoDS_Shape& cutter)
{
    return new TopoDS_Shape(BRepAlgoAPI_Cut(result, cutter).Shape());
}
